...
var Clip = function (liveosc, track, id) {
  ...
  function loopstartListener(trackId, clipId, loopstart) {
    ...
    self.eventEmitter.emit('loopstart', {
      value: loopstart,
      prev: self.loopstart
    });
  ...
  function loopendListener(trackId, clipId, loopend) {
    if (self.track.id != trackId) return;
    if (self.id != clipId) return;
    self.eventEmitter.emit('loopend', {
      value: loopend,
      prev: self.loopend
    });
  ...
  function loopstateListener(trackId, clipId, loopstate) {
    if (self.track.id != trackId) return;
    if (self.id != clipId) return;
    self.eventEmitter.emit('loopstate', {
      value: loopstate,
      prev: self.loopstate
    });
  ...
  function warpingListener(trackId, clipId, warping) {
    if (self.track.id != trackId) return;
    if (self.id != clipId) return;
    self.eventEmitter.emit('warping', {
      value: warping,
      prev: self.warping
    });
  ...
  function pitchListener(trackId, clipId, coarse, fine) {
    if (self.track.id != trackId) return;
    if (self.id != clipId) return;
    self.eventEmitter.emit('coarse', {
      value: coarse,
      prev: self.coarse
    });
    self.eventEmitter.emit('fine', {
      value: fine,
      prev: self.fine
    });
